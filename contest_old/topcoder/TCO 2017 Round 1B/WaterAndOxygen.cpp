...
class WaterAndOxygen {
    ...
    double maxDays(int remainH20, int remainO2, int costH2O, int costO2) {
        double x = remainO2 * 1.0 / costO2, y = remainH20 * 1.0 / costH2O;
        if(x > y) return y;
        double leftH2O = remainH20 - costH2O * x;
        double day = x + leftH2O*1. / (costH2O + costO2*2.);
        return day;
    }
};
