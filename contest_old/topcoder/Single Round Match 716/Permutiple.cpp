...
class Permutiple {
    ...
    string isPossible(int x) {
        int t = 0, y = x;
        if(x < 10) return "Impossible";
        while(y) dg[y%10] ++, y /= 10, t++;
        if(dg[0] == t-1) return "Impossible";
        if(x % 10 == 0) return "Possible";
        for(int i = 2; i < 11; ++i) {
            int z = x*i, u = 0;
            zero(g);
            while(z) g[z%10]++, z /= 10, u++;
            if(u > t) return "Impossible";
            int f = 1;
            for(int j = 0; j < 10; ++j) if(dg[j] != g[j]) {
                f = 0;
                break;
            }
            if(f) break;
