...
class RelativeHeights {
    ...
    int countWays(vector<int> h) {
        vector<pii > v;
        rep(i, 0, sz(h)) v.pb({h[i], i});
        sort(v.begin(), v.end());
        // reverse(v.begin(), v.end());
        rep(i, 0, sz(v)) h[v[i].se] = i;
        for(auto & u : h) cout << u << " "; cout << endl;
        int k = 0;
        rep(i, 0, sz(h)) {
            if(i && h[i]-h[i-1] == 1) continue;
            if(i < sz(h)-1 && h[i]-h[i+1] == 1) continue;
            ++k;
        }
        return k;
    }
